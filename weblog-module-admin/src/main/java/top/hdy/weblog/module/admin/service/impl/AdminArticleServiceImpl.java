...
import top.hdy.weblog.module.admin.model.vo.article.FindArticlePageListRspVO;
...
import top.hdy.weblog.module.common.domain.dos.*;
import top.hdy.weblog.module.common.domain.mapper.*;
...
public class AdminArticleServiceImpl implements AdminArticleService {
     ...
     */
    @Override
    // 表示在该方法执行之前，开启一个事务，保证方法内所有操作是个原子性的，要不全部成功，要不全部失败，一旦中间发生异常，则回滚事务。
    ...
    private void insertTags(Long articleId, List<String> publishTags) {
        ...
        if (CollectionUtils.isEmpty(tagDOS)) {
            ...
            List<String> tagIds = tagDOS.stream()
                    .map(tagDO -> String.valueOf(tagDO.getId()))
                    .collect(Collectors.toList());
            // 表中已添加相关标签，则需要筛选
            // 通过标签 ID 来筛选，包含对应 ID 则表示提交的标签是表中存在的
            existedTags = publishTags.stream()
                    .filter(publishTag -> tagIds.contains(publishTag))
                    .collect(Collectors.toList());
            // 否则则是不存在的
            notExistTags = publishTags.stream()
                    .filter(publishTag -> !tagIds.contains(publishTag))
                    .collect(Collectors.toList());
            // 补充逻辑：
            ...
            while (iterator.hasNext()) {
                ...
                if (tagNameIdMap.containsKey(notExistTag.toLowerCase())) {
                    //从不存在的标签集合中删除
                    iterator.remove();
                    notExistTags.add(String.valueOf(tagNameIdMap.get(notExistTag.toLowerCase())));
                }
            }
        }
